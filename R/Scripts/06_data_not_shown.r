...
#- 6.2.1: PE rate in ligation patients
ligation_PE_rate <- final %>%
  ...
  summarise(
    ...
    pct_vte = round(n_vte / n_total * 100, 0),
    n_pct_vte   = paste0(n_vte, " (", pct_vte, "%)"),
    .groups = "drop"
  )
#- 6.2.2: PPX strategy in ligation patients
ligation_ppx_strategy <- final %>%
  filter(IVC_repair_type == "Ligation", analyze == "Y") %>%
  mutate(
    ppx_strategy = case_when(
      AT_VTE == "+ASA, -VTE, +PPX, -VTE" ~ "PAC+AP",
      AT_VTE == "-ASA, -VTE, +PPX, -VTE" ~ "PAC",
      TRUE ~ "Other"
    )
  ) %>%
  count(ppx_strategy) %>%
...
#- 6.4.1: DC AC groupings and ANY_PPX grouping
...
#- 6.5.2: +PPX patients heparin breakdown
ppx_heparin_type <- heparin_info |>
  filter(PPX_Status == "+PPX") |>
  count(heparin_type) |>
  mutate(pct = round(n/sum(n)*100, 0), pct_n_N = paste0(pct, "% (", n, "/", sum(n), ")")) |>
  select(heparin_type, pct_n_N)
